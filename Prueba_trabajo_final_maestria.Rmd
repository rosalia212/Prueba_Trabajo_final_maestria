...
paquetes_necesarios <- c(
  ...
  "ggrepel", "countrycode", "scales", "maps", "DT", "lorem", "png", 
  "ggcorrplot","grid", "corrplot", "patchwork", "FactoMineR","ppcor",
  "GGally","ranger","factoextra","keras","pROC","reticulate"
)
...
lapply(paquetes_necesarios, instalar_si_falta)
...
```{r}
getwd()
Datos_APILOL <- read_csv("/Users/lizeth.herrera/Documents/Master/Repositorio_Rosalia/Datos/ryse_database.csv")
Datos <- Datos_APILOL
```
```{r}
#Depuración de datos
...
```{r}
#OCI: indice de control de objetivos
df <- df %>%
  mutate(
...
```{r}
#importancias no llineales(random Forest)
library(ranger)
...
```{r}
#Correlaciones y correlacion parcial (controlando duración)
library(GGally)
df |> select(win, gpm, kda_adj, oci, vision_min, dmg_dealt_min, cc_min) |> GGally::ggpairs()

# install.packages("ppcor")
library(ppcor)
pcor.test(df$win, df$oci, df$time_played)   # correlación parcial win–oci | duración
```


```{r}
#Medir si algún rol pesa más que otro
m2 <- glm(win ~ gpm + kda_adj + oci*individual_position + vision_min,
          data = df, family = binomial())
broom::tidy(m2)
```
...
consistencia <- df %>%
  ...
  summarise(
    n_matches = n(),
    gpm_mean  = mean(gpm, na.rm=TRUE),
    gpm_sd    = sd(gpm, na.rm=TRUE),
    kda_mean  = mean(kda_adj, na.rm=TRUE),
    kda_sd    = sd(kda_adj, na.rm=TRUE),
    oci_mean  = mean(oci, na.rm=TRUE),
    oci_sd    = sd(oci, na.rm=TRUE),
    winrate   = mean(win, na.rm=TRUE)
  ) %>%
  filter(n_matches >= 10) %>%
  mutate(
    gpm_cv = gpm_sd/pmax(gpm_mean, 1e-6),
    kda_cv = kda_sd/pmax(kda_mean, 1e-6),
    oci_cv = oci_sd/pmax(oci_mean, 1e-6)
  )
...
# 1) Winrate ~ gpm_cv
p_gpm <- ggplot(consistencia, aes(x = gpm_cv, y = winrate)) +
  geom_point(alpha = 0.6) +
  stat_smooth(method = "lm", se = FALSE) +
  labs(title = "Winrate vs Consistencia en GPM",
       x = "Coeficiente de variación de GPM (gpm_cv)",
       y = "Winrate") +
  theme_minimal(base_size = 12)
p_gpm

# 2) Winrate ~ kda_cv
p_kda <- ggplot(consistencia, aes(x = kda_cv, y = winrate)) +
  geom_point(alpha = 0.6) +
  stat_smooth(method = "lm", se = FALSE) +
  labs(title = "Winrate vs Consistencia en KDA",
       x = "Coeficiente de variación de KDA (kda_cv)",
       y = "Winrate") +
  theme_minimal(base_size = 12)
p_kda

# 3) Winrate ~ oci_cv
p_oci <- ggplot(consistencia, aes(x = oci_cv, y = winrate)) +
  geom_point(alpha = 0.6) +
  stat_smooth(method = "lm", se = FALSE) +
  labs(title = "Winrate vs Consistencia en OCI",
       x = "Coeficiente de variación de OCI (oci_cv)",
       y = "Winrate") +
  theme_minimal(base_size = 12)
p_oci
```
...
```{r}
#Perfiles de estilo (PCA +k means)
library(dplyr)
library (FactoMineR); library(factoextra)

X <- df %>% 
  dplyr::select(gpm, kda_adj, oci, vision_min, dmg_dealt_min, dmg_taken_min, cs_min, cc_min) %>% scale()
pca <- PCA(X, graph = FALSE)
set.seed(123)
km  <- kmeans(pca$ind$coord[,1:3], centers = 4, nstart = 25)
df$cluster <- factor(km$cluster)

df %>%
  group_by(cluster) %>%
  summarise(winrate = mean(win),
...
gam1 <- gam(win ~ s(gpm) + s(kda_adj) + s(oci) + s(vision_min),
            data = df, family = binomial())
summary(gam1)
plot(gam1, pages = 1, shade = TRUE)   # ver formas de las curvas
```


```{r}
library(ggplot2)
p1 <- ggplot(df, aes(tower_kills, win)) +
  geom_smooth(method="glm", method.args=list(family=binomial()), se=FALSE) +
  geom_point(alpha=.1) + labs(title="Win ~ Torres")

p2 <- ggplot(df, aes(tower_kills, inhibitor_kills)) +
  geom_smooth(se=FALSE) + geom_point(alpha=.1) + labs(title="Inhibidores ~ Torres")

p3 <- ggplot(df, aes(inhibitor_kills, win)) +
  geom_smooth(method="glm", method.args=list(family=binomial()), se=FALSE) +
  geom_point(alpha=.1) + labs(title="Win ~ Inhibidores")
print(p1)
print(p2)
print(p3)
...
```{r}
library(keras)
#install_keras()  # solo la primera vez
library(dplyr)

cols <- c("win","gpm","kda_adj","oci","vision_min",
          "dmg_dealt_min","dmg_taken_min","cs_min","cc_min")

vars_modelo <- df[, cols]   # toma solo esas columnas


# Quitar NAs
vars_modelo <- vars_modelo[stats::complete.cases(vars_modelo), ]

# Escalado de X (manteniendo 'win' sin escalar)
X <- scale(vars_modelo[, setdiff(cols, "win"), drop = FALSE])
y <- vars_modelo[, "win"]
...
train_df <- data.frame(win = y_train, X_train)
test_df  <- data.frame(win = y_test,  X_test)


# --- Red neuronal con 'nnet' (sin TensorFlow) ---
# Si no lo tienes: install.packages("nnet")
library(nnet)

# size = neuronas en la capa oculta; decay = regularización L2
set.seed(123)
nn <- nnet::nnet(
  win ~ .,
  data   = train_df,
  ...
  decay  = 1e-4,
  maxit  = 500,
  linout = FALSE,   # salida logística (clasificación)
  trace  = FALSE
)

# Probabilidades de victoria en test
pred_prob <- predict(nn, newdata = test_df, type = "raw")
pred_cls  <- ifelse(pred_prob >= 0.5, 1, 0)

# Métricas básicas
cat("Accuracy:", mean(pred_cls == y_test), "\n")
table(Predicho = pred_cls, Real = y_test)


# --- Curva ROC y AUC ---
# Si no lo tienes: install.packages("pROC")
library(pROC)
...
plot(roc_obj, col = "steelblue", lwd = 2, main = paste("ROC - AUC =", round(auc_val, 3)))
